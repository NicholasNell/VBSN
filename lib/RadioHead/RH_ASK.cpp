...
RH_ASK::RH_ASK(
	...
	uint8_t preamble[RH_ASK_PREAMBLE_LEN] = { 0x2a, 0x2a, 0x2a, 0x2a, 0x2a,
												0x2a, 0x38, 0x2c };
	memcpy(_txBuf, preamble, sizeof(preamble));
}

bool RH_ASK::init( ) {
	if (!RHGenericDriver::init()) return false;
	thisASKDriver = this;
...
void RH_ASK::validateRxBuf( ) {
	...
	if (_promiscuous
			|| _rxHeaderTo
					== _thisAddress||_rxHeaderTo == RH_BROADCAST_ADDRESS) {
		_rxGood++;
		_rxBufValid = true;
	}
}
...
void RH_INTERRUPT_ATTR RH_ASK::receiveTimer( ) {
	...
	if (rxSample) _rxIntegrator++;

	if (rxSample != _rxLastSample) {
		// Transition, advance if ramp > 80, retard if < 80
		_rxPllRamp +=
				((_rxPllRamp < RH_ASK_RAMP_TRANSITION) ?
						RH_ASK_RAMP_INC_RETARD : RH_ASK_RAMP_INC_ADVANCE);
	...
	if (_rxPllRamp >= RH_ASK_RX_RAMP_LEN) {
		...
		if (_rxIntegrator >= 5) _rxBits |= 0x800;
		...
		if (_rxActive) {
			...
			if (++_rxBitCount >= 12) {
				...
				uint8_t this_byte = (symbol_6to4(_rxBits & 0x3f)) << 4
...
void RH_INTERRUPT_ATTR RH_ASK::transmitTimer( ) {
	...
	if (_txSample > 7) _txSample = 0;
}

void RH_INTERRUPT_ATTR RH_ASK::handleTimerInterrupt( ) {
	if (_mode == RHModeRx)
		receiveTimer(); // Receiving
	else if (_mode == RHModeTx) transmitTimer(); // Transmitting
}
